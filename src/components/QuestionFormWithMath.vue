<template>
      <form @submit.prevent="handleSubmit" class="needs-validation" novalidate>
        <div class="form-section mb-4">
          <h3 class="section-subtitle mb-3">Question Settings With Quill</h3>
              ...
              <div class="flex-grow-1">
                ...
                <template v-if="form.is_math">
                     <div class="mb-2">
                        ...
                        <ckeditor :config="editorConfig" v-model="form.questionText" id="editor1" />
                    </div>
              ...
              <div v-for="(option, idx) in form.options" :key="idx" class="input-group input-group-sm mb-2" style="max-width:440px;">
                ...
                <template v-if="form.is_math">
                  <math-field
                    ...
                    @input="onOptionMathInput(idx, $event)"
                  ></math-field>
                </template>
                <template v-else>
            ...
            <div class="flex-grow-1">
              <template v-if="form.is_math">
                ...
                <div class="form-text mt-1">
                  ...
                  Use LaTeX syntax for equations (e.g., \frac{1}{2} for fractions)
                </div>
              </template>
              <template v-else>
        ...
        <div v-else-if="questionType === 'short_answer'" class="card p-3 mb-3">
          ...
          <div class="mb-3">
            ...
            <template v-if="form.is_math">
              ...
              <div class="form-text mt-1">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4m0-4h.01"/></svg>
                Students must match this equation exactly. Use LaTeX syntax (e.g., \frac{1}{2})
              </div>
            </template>
            <template v-else>
...
export default {
    ...
    return {
      form: this.editQuestion ? { ...this.editQuestion } : {
        ...
        editorConfig: {
            ...
            removePlugins: 'mathjax', // optional: remove if switching fully to MathType
            height: 50,
            allowedContent: true,
            extraAllowedContent: 'math[*]{*};img[data-mathml,data-custom-editor,role](Wirisformula)',
            toolbar: [
                ['Undo', 'Redo'],
  ...
  mounted() {
    ...
    CKEDITOR.plugins.addExternal(
        'ckeditor_wiris',
        '/ckeditor/@wiris/mathtype-ckeditor4/',
        'plugin.js'
    );
  ...
  methods: {
    onMathInput(e) {
      this.form.questionMath = e.target.value;
    },
    onOptionMathInput(idx, e) {
      this.form.options[idx] = e.target.value;
    },
    addOption() {
    ...
    async handleSubmit() {
      ...
      if (this.form.is_math) {
        finalQuestion = (this.form.questionText || '') ;
      } else {
        finalQuestion = this.form.question;
      }
      // Validation
